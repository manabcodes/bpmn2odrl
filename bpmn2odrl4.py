...
def classify_obligations(start:     str,
                         dag_nodes: List[str],
                         dag_succ:  Dict[str, List[str]],
                         ) -> Set[str]:
    """
    Returns the set of node-ids that are OBLIGATIONS (odrl:Duty).
    All other nodes are PERMISSIONS (odrl:Permission).

    A node is an obligation if and only if removing it from the graph
    makes every end node unreachable from start — meaning there is no
    complete start-to-end path that bypasses it.

    Algorithm: one BFS per node
    ----------------------------
    1. BFS from start over full graph → find all reachable end nodes
       (nodes with no successors).
    2. For each node T:
         a. BFS from start, excluding T
         b. If no end node reachable → T is on every path → Duty
         c. If any end node still reachable → T can be skipped → Permission

    Complexity: O(V x (V+E)) — fine for BPMN sizes.
    """
    all_reachable = _bfs_reachable(start, dag_succ)
    end_nodes = {v for v in all_reachable if not dag_succ.get(v)}

    if not end_nodes:
        return set(all_reachable)
...
def print_summary(rules: List[dict], g: BPMNGraph):
    ...
    print(f"\n  Totals: {len(duties)} Duties (obligatory), "
          f"{len(perms)} Permissions (skippable)")
    print("═" * 72 + "\n")
...
def run_pipeline(xml_path: str,
    ...
    # Run dominance tree and obligation classification per root, then merge
    all_idom: Dict[str, Optional[str]] = {}
    all_cp:   Set[str] = set()
    for sm in start_metas:
        all_idom.update(build_dominance_tree(sm, dag_nodes, dag_succ))
        all_cp  |= classify_obligations(sm, dag_nodes, dag_succ)

    duty_tasks = [g.nodes[n]["label"] for n in all_cp
                  if g.nodes.get(n, {}).get("kind") == "task"]
    perm_tasks = [g.nodes[n]["label"] for n in dag_nodes
                  if g.nodes.get(n, {}).get("kind") == "task"
                  and n not in all_cp]
    if verbose:
        print(f"      Duties (all pools):      {duty_tasks}")
        print(f"      Permissions (all pools): {perm_tasks}")

    print(f"[3/6] Dominance-tree construction ({len(start_metas)} root(s))")
    print(f"[4/6] Deontic classification — "
          f"{len(duty_tasks)} Duties, {len(perm_tasks)} Permissions")
