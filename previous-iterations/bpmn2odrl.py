...
def role_partitioned_dfs(
    ...
    """
    DFS over the DAG, partitioned by swim lane.
    Emits a list of ODRL rule dicts.

    Each dict has:
      uid         : unique rule identifier (URI fragment)
      type        : "Duty" | "Permission" | "Prohibition"
      action      : natural-language label of the task
      assignee    : participant name (party performing the action)
      precondition: list of rule-uids that must hold before this one (from idom)
      constraints : list of string conditions (from XOR gateway labels)
      inter_party : list of {duty_type, other_party, trigger} for cross-lane duties
      gateway_type: None | "XOR" | "AND" | "OR"  (if this is a gateway node)
    """
    rules: List[dict]   = []
    uid_map: Dict[str, str] = {}   # meta-node-id → rule uid
    visited: Set[str]   = set()
    # Track constraint stack: accumulated XOR conditions along the current path
    constraint_stack: List[Tuple[str, str]] = []   # (gateway_id, condition_label)

    # Pre-build message-flow lookup by meta-node
    # outgoing msg: meta_src → [(meta_tgt, flow_id)]
    # incoming msg: meta_tgt → [(meta_src, flow_id)]
    ...
    for (src, tgt, fid) in g.msg_flows:
        ...
        # Skip pool-level endpoints (participants not in node map)
        if ms in g.nodes and mt in g.nodes:
            msg_out[ms].append((mt, fid))
            msg_in[mt].append((ms, fid))

    # Build condition map: (meta_src, meta_tgt) → condition label
    # (last writer wins for multiple flows between same pair)
    cond_map: Dict[Tuple[str, str], str] = {}
    for (src, tgt, cond) in g.seq_edges:
        ms = node_to_meta.get(src, src)
        mt = node_to_meta.get(tgt, tgt)
        if cond:
            cond_map[(ms, mt)] = cond

    def dfs(v: str, conditions: List[str]):
        if v in visited:
            return
        visited.add(v)
        ...
        # Gateways: propagate conditions to each outgoing branch, emit no rule
        if kind == "gateway":
            for w in dag_succ.get(v, []):
                edge_cond = cond_map.get((v, w), "")
                new_conds = conditions + ([edge_cond] if edge_cond else [])
                if w not in visited:
                    dfs(w, new_conds)
            return

        if kind in ("startEvent", "endEvent", "event"):
            # Recurse but emit no rule
            for w in dag_succ.get(v, []):
                if w not in visited:
                    dfs(w, conditions)
            return

        # ── Task / meta_loop node → emit a rule ──────────────────────────
        rule_type  = "Duty" if v in cp_nodes else "Permission"
        uid        = slugify(label) + "_" + v[-6:]
        uid_map[v] = uid

        # Precondition from immediate dominator (if it was a task, not a gateway/event)
        idom_v    = idom.get(v)
        pre_uid   = uid_map.get(idom_v) if idom_v else None

        # Inter-party relationships from message flows
        inter: List[dict] = []
        for (mt, fid) in msg_out.get(v, []):
            tgt_nd   = g.nodes.get(mt, {})
            tgt_pid  = tgt_nd.get("participant", "unknown")
        ...
        for (ms, fid) in msg_in.get(v, []):
            ...
            src_pid = src_nd.get("participant", "unknown")
